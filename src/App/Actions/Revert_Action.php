...
class Revert_Action extends Runq_Action {
	public function execute(Request $Request): Request_Response {
		...
		if (!in_array($file, $processed)) {
			$this->Printer->error('File not processed: ' . $file_path);
			$this->exitNow(1);
		}
		...
		// remove file from processed
		$key = array_search($file, $log['processed']);
		if ($key !== false) {
			unset($log['processed'][$key]);
		}
